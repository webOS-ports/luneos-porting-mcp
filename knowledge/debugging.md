...
- **netconsole (Tier B only)** — after the debug network is up, init attaches a dynamic netconsole target streaming kmsg to UDP broadcast port 6666; the host listens with `nc -ul 6666`, and the stream **survives switch_root into the full boot**. Requires `CONFIG_NETCONSOLE=y` + `CONFIG_NETCONSOLE_DYNAMIC=y`, now in the athena and mindphone fragments. **Do not add it to a Tier A GKI fragment: NETCONSOLE selects NETPOLL, which adds a field to `struct net_device` — KMI-poison of the SYSVIPC class.** Prior art: the tenderloin mainline kernel bakes `netconsole=6665@172.16.42.2/usb0,6666@172.16.42.1/` into `CONFIG_CMDLINE` (note its webOS-heritage `172.16.42.x` addressing) — it gets *pre-userspace* early boot logging because its mainline kernel owns the gadget from boot, which a configfs-gadget device cannot match; its custom initramfs (`touchpad-kernel/initramfs-working/`) also runs telnetd at `172.16.42.2`.
